...
std::vector<Room*> Algorithm::findShortestPath(Room* start, Room* end) {
    ...
    while (!pq.empty()) {
        ...
        // Explore neighbors
        for (Room* neighbor : current->exits) {
            // Ignore full rooms unless it is the destination
            if (neighbor->isfull && neighbor != end) {
                continue;
            }
...
std::vector<std::vector<Room*>> Algorithm::findOptimizedRoutes(Room* start, Room* end) {
    ...
    std::vector<Room*> blockedRooms;
    bool directConnectionBlocked = false;
    ...
    while (true) {
        ...
        if (path.size() == 2) {
            // Temporarily remove the direct link to avoid infinite loop
            auto it1 = std::find(start->exits.begin(), start->exits.end(), end);
            if (it1 != start->exits.end()) {
                start->exits.erase(it1);
            }
            auto it2 = std::find(end->exits.begin(), end->exits.end(), start);
            if (it2 != end->exits.end()) {
                end->exits.erase(it2);
            }
            ...
            for (size_t i = 1; i < path.size() - 1; ++i) {
                path[i]->isfull = true;
                blockedRooms.push_back(path[i]);
            }
        }
    }

    // Restore blocked rooms to their original state
    for (Room* r : blockedRooms) {
        r->isfull = false;
    }

    // Restore the direct connection if it was blocked
        ...
        for (size_t r = 0; r < routes.size(); ++r) {
            ...
            for (int a = 0; a < numAnts; ++a) {
                ...
                if (currentPos > 0 && currentPos < (int)routes[r].size() - 1) {
                    ...
                    std::string roomName = routes[r][positions[r][a]]->name;
            ...
            if (nextIdx < antsOnRoute[r].size()) {
                ...
                std::string roomName = routes[r][1]->name;
